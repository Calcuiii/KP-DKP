    ...
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8">
        <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
            <div class="max-w-3xl">
                ...
                <h2 class="mt-2 text-2xl font-bold sm:text-3xl">Pemeriksaan Surat Permohonan</h2>
        ...
        <div class="mt-7 rounded-3xl border border-ocean/15 bg-gradient-to-br from-ocean/[0.05] to-teal/[0.05] p-5 sm:p-6">
            <div class="flex items-start gap-3">
                ...
                <div><h3 class="text-base font-bold">Informasi yang wajib tercantum</h3><p class="mt-1 text-xs leading-relaxed text-muted-foreground">Gunakan daftar ini untuk memeriksa surat sebelum diunggah.</p></div>
            </div>
                ...
                @foreach ($requirements as $index => [$icon, $label])
                    <li class="rounded-2xl border border-ocean/10 bg-white/90 p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-ocean/25 hover:shadow-md">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="rounded-full bg-navy px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white">{{ chr(97 + $index) }}</span>
                        </div>
                        <p class="mt-4 text-xs font-bold leading-relaxed text-navy">{{ $label }}</p>
            ...
            <div class="max-w-3xl">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h2 class="mt-1 text-2xl font-bold">Isi dan buktikan Form WOPPS</h2>
                    </div>
                </div>
        ...
        @if ($ethicsApproved)
            <div class="mt-7 grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
                <div class="rounded-3xl bg-gradient-to-br from-navy via-[#123d72] to-ocean p-6 text-white">
                    ...
                    <h3 class="mt-5 text-lg font-bold">Form resmi WOPPS</h3>
                    ...
                    <a href="https://bit.ly/WOPPS" target="_blank" rel="noopener noreferrer" class="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-ocean transition hover:-translate-y-0.5 hover:shadow-lg">
                ...
                <div class="rounded-3xl border border-ocean/15 bg-background p-5 sm:p-6">
                    @if ($woppsFormCompleted)
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <h3 class="text-sm font-bold">Bukti pengisian sudah dikirim</h3>
                    ...
                    @else
                        <h3 class="text-sm font-bold">Upload bukti pengiriman</h3>
    ...
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $letterApproved ? '' : 'opacity-60' }}">
        <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
            <div class="max-w-3xl">
                <div class="flex items-center gap-3">
                    ...
                    <div><p class="text-xs font-bold uppercase tracking-[0.18em] text-teal">WOPPS · Tahap 2</p><h2 class="mt-1 text-2xl font-bold">Pemeriksaan Ethics Approval Statement Letter</h2></div>
                </div>
        ...
        <div class="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            @foreach ([
                ...
                <div class="rounded-2xl border border-ocean/10 bg-light/50 p-4"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-ocean/10 text-ocean"><i data-lucide="{{ $icon }}" class="h-4 w-4"></i></span><h3 class="mt-3 text-xs font-bold">{{ $title }}</h3><p class="mt-1 text-[11px] leading-relaxed text-muted-foreground">{{ $description }}</p></div>
            @endforeach
        </div>
...
<section id="tindak-lanjut-wopps" class="scroll-mt-28">
    <article class="relative overflow-hidden rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $woppsFormCompleted ? '' : 'opacity-60' }}">
            ...
            <div class="max-w-3xl">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h2 class="mt-1 text-2xl font-bold">Keputusan dan surat balasan Dinas</h2>
                    </div>
                </div>
        ...
        @if ($woppsFormCompleted)
            @if($accepted)
                <div class="mt-7 rounded-3xl border border-teal/25 bg-teal/[0.06] p-6"><p class="text-xs font-bold uppercase tracking-wider text-teal">Pengajuan diterima</p><h3 class="mt-2 text-xl font-bold">Layanan WOPPS Anda dapat ditindaklanjuti.</h3><p class="mt-2 text-sm text-muted-foreground">Surat balasan resmi sudah tersedia. Tidak ada periode tanggal untuk layanan WOPPS.</p>@if($replyLetter)<a href="{{ route('peserta.response-letter.download') }}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-navy px-5 py-3 text-sm font-bold text-white"><i data-lucide="download" class="h-4 w-4"></i>Unduh Surat Balasan</a>@endif</div>
            @elseif($rejected)
                <div class="mt-7 rounded-3xl border border-red-200 bg-red-50 p-6"><p class="text-xs font-bold uppercase tracking-wider text-red-700">Pengajuan ditolak</p><h3 class="mt-2 text-xl font-bold">Pengajuan WOPPS ini telah ditutup.</h3><p class="mt-2 text-sm text-red-800">Keputusan ini bersifat final. Dokumen pada pengajuan ini tidak dapat diperbaiki kembali.</p>@if($replyLetter)<a href="{{ route('peserta.response-letter.download') }}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-navy px-5 py-3 text-sm font-bold text-white"><i data-lucide="download" class="h-4 w-4"></i>Unduh Surat Balasan</a>@endif</div>
            @else
                <div class="mt-7 rounded-3xl border border-amber-200 bg-amber-50 p-6"><p class="text-sm font-bold text-amber-900">Menunggu keputusan Dinas</p><p class="mt-2 text-sm leading-relaxed text-amber-800">Surat balasan akan tersedia di portal dan dikirim melalui email setelah Dinas menetapkan keputusan.</p></div>
            @endif
        @endif
    </article>
</section>
...
<section id="penyelesaian-wopps" class="scroll-mt-28">
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $accepted || $rejected ? '' : 'opacity-60' }}">
        ...
        <h2 class="mt-2 text-2xl font-bold">Tindak lanjut dan penyelesaian</h2>
        @if($accepted)
            @if($completed)
                <div class="mt-6 rounded-3xl border border-emerald-200 bg-emerald-50 p-6"><p class="text-sm font-bold text-emerald-800">Layanan WOPPS selesai</p><p class="mt-2 text-sm text-emerald-700">Diselesaikan pada {{ $application->completed_at->translatedFormat('d F Y, H:i') }} WIB. Anda dapat mengajukan layanan baru melalui akun yang sama.</p></div>
            @elseif($contacted)
                <div class="mt-6 rounded-3xl border border-teal/25 bg-teal/[0.06] p-6"><p class="text-sm font-bold text-teal">Anda sudah dihubungi petugas Dinas</p><p class="mt-2 text-sm text-muted-foreground">Silakan mengikuti arahan petugas. Status selesai akan diperbarui oleh Dinas setelah layanan tuntas.</p></div>
            @else
                <div class="mt-6 rounded-3xl border border-ocean/20 bg-ocean/[0.05] p-6"><p class="text-sm font-bold text-ocean">Menunggu dihubungi pihak Dinas</p><p class="mt-2 text-sm leading-relaxed text-muted-foreground">Anda tidak perlu menghubungi petugas terlebih dahulu. Pastikan nomor telepon pada Form WOPPS aktif; petugas Dinas akan menghubungi Anda.</p></div>
            @endif
        @elseif($rejected)
            <div class="mt-6 rounded-3xl bg-slate-50 p-6"><p class="text-sm font-bold">Pengajuan telah ditutup</p><p class="mt-2 text-sm text-muted-foreground">Anda dapat membuat pengajuan baru tanpa mengubah riwayat pengajuan yang ditolak.</p></div>
        @else
            <p class="mt-4 text-sm text-muted-foreground">Tahap ini tersedia setelah keputusan Dinas diterbitkan.</p>
        @endif

        @if($completed || $rejected)
            <div class="mt-6 rounded-3xl bg-gradient-to-br from-navy to-ocean p-6 text-white"><h3 class="text-lg font-bold">Ingin mengajukan layanan lagi?</h3><p class="mt-2 text-sm text-blue-100">Pilih layanan baru. Riwayat pengajuan ini tetap tersimpan dan tidak akan diubah.</p><form method="POST" action="{{ route('peserta.application.store') }}" class="mt-5 flex flex-wrap gap-3">@csrf<button name="service_type" value="magang_pkl" class="rounded-xl bg-white px-4 py-3 text-sm font-bold text-ocean">Magang / PKL / KP</button><button name="service_type" value="wopps" class="rounded-xl border border-white/30 bg-white/10 px-4 py-3 text-sm font-bold text-white">WOPPS</button></form></div>
        @endif
    </article>
</section>

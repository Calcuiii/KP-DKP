@php
    ...
    */
    ...
    $guestbookProof = $application->latestDocument(
        ParticipantApplicationDocument::TYPE_GUESTBOOK
    );

    $requestLetter = $application->latestDocument(
        ParticipantApplicationDocument::TYPE_REQUEST_LETTER
    );

    $internshipFormProof = $application->latestDocument(
    ...
    */
    ...
    $letterNeedsRevision =
        $requestLetter?->review_status ===
        ParticipantApplicationDocument::REVIEW_REVISION;

    $automatedNeedsRevision = in_array(
        $requestLetter?->automated_check_status,
    ...
    */
    ...
    $certificateFollowUpRequired =
        $application->certificateFollowUpRequired();

    $certificateFollowUpChoice =
        $application->certificate_follow_up_choice;
    ...
    | Ini yang penting.
    |
    | Google Form TIDAK hanya bergantung kepada surat approved.
    |
    | Kalau surat approved tetapi certificate_eligible = false,
    ...
    */
    ...
    $canUploadLetter =
        ...
        || (
            ...
            && $requestLetter?->certificate_eligible === false
            && $certificateFollowUpChoice === 'upload_again'
        );
    ...
    */
    ...
    $daysRemaining = $officialEnded
        ? max(
            0,
            $today->diffInDays(
                $officialEnded->copy()->startOfDay(),
                false
            )
        )
        : null;

    $availableCount = $locations
    ...
    */
    ...
    $normalizedDecision =
        mb_strtolower((string) $application->decision);

    $applicationAccepted = in_array(
        $normalizedDecision,
        ['accepted', 'approved', 'diterima'],
        true
    );

    $applicationRejected = in_array(
        $normalizedDecision,
    ...
    */
    ...
    $stageSixUnlocked =
        $officialStarted !== null
        || $applicationAccepted;

    $calendarMonth =
        $officialStarted && $officialEnded
            ? (
                $today->betweenIncluded(
                    $officialStarted->copy()->startOfDay(),
                    $officialEnded->copy()->startOfDay()
                )
                    ? $today->copy()
                    : $officialStarted->copy()
            )->startOfMonth()
            : null;

    $calendarDays = $calendarMonth
        ? \Carbon\CarbonPeriod::create(
            $calendarMonth->copy()->startOfWeek(
                \Carbon\CarbonInterface::MONDAY
            ),
            $calendarMonth->copy()->endOfMonth()->endOfWeek(
...
<section id="persiapan">
    ...
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8">
        ...
        <div class="flex items-start gap-4">
            ...
            <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl {{ $guestbookProof ? 'bg-teal text-white' : 'bg-ocean/10 text-ocean' }}">
            ...
            <div>
                ...
                <p class="mt-2 text-sm leading-relaxed text-muted-foreground">
                    Pengisian dilakukan secara individu.
                    Setelah selesai, unggah screenshot atau PDF sebagai bukti.
                </p>
        ...
        @if ($guestbookProof)
            ...
            <div class="mt-6 flex flex-col gap-4 rounded-2xl border border-teal/25 bg-teal/[0.06] p-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <i data-lucide="check-circle" class="h-5 w-5 text-teal"></i>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">
                            {{ $guestbookProof->original_name }}
                            ·
                            {{ $guestbookProof->created_at->format('d M Y, H:i') }}
                        </p>
                ...
                <a
                    href="{{ route('peserta.document.download', $guestbookProof) }}"
                    class="text-xs font-bold text-ocean"
                >
                    Lihat bukti
                </a>
        ...
        @else
            ...
            <div class="mt-6 rounded-2xl border border-ocean/20 bg-ocean/[0.05] p-4 text-sm leading-relaxed text-muted-foreground">
                ...
                Setelah selesai mengisi, kembali ke dashboard ini
                untuk mengunggah bukti pengisian.
            ...
            <form
                method="POST"
                action="{{ route('peserta.guestbook-proof.store') }}"
                enctype="multipart/form-data"
                class="mt-5 rounded-2xl border border-dashed border-ocean/30 bg-background p-5"
            >

                @csrf

                <label class="text-sm font-bold">
                    Upload bukti pengisian
                </label>

                <input
                    type="file"
                    name="guestbook_proof"
                    accept=".jpg,.jpeg,.png,.pdf"
                    required
                    class="mt-3 block w-full rounded-xl border border-border bg-white p-3 text-sm"
                >

                @error('guestbook_proof')
                ...
                <label class="mt-4 flex items-start gap-3 text-xs leading-relaxed text-muted-foreground">
                    ...
                    <input
                        type="checkbox"
                        name="guestbook_declaration"
                        value="1"
                        required
                        class="mt-0.5"
                    >

                    Saya menyatakan telah mengisi Buku Tamu
                    dengan data yang benar.

                </label>

                <button
                    class="mt-4 rounded-xl bg-ocean px-5 py-2.5 text-sm font-bold text-white"
                >
                    Simpan Bukti
                </button>
        ...
        <div class="mt-6 flex items-start gap-3 rounded-2xl border border-ocean/15 bg-ocean/[0.05] p-4">
            ...
            <div>
                ...
                <p class="mt-1 text-xs leading-relaxed text-muted-foreground">
                    Buku Tamu menjadi pendataan awal peserta sebelum surat diproses.
                    Bukti dapat berupa screenshot halaman selesai atau PDF respons pengisian,
                    maksimal 5 MB.
                </p>
...
<section
    id="lokasi-kuota"
    class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $guestbookProof ? '' : 'opacity-60' }}"
>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div>
            ...
            <p class="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
                Gunakan informasi ini untuk menentukan lokasi tujuan di dalam surat.
                Status kuota bukan jaminan penerimaan.
            </p>

        </div>

        <span class="rounded-full bg-teal/10 px-4 py-2 text-xs font-bold text-teal">
            {{ $availableCount }} lokasi tersedia
        </span>
    ...
    <div
        ...
        aria-label="Keterangan status kuota"
    >

        <span class="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-[11px] font-bold text-emerald-700">
            <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
            Kuota tersedia
        </span>

        <span class="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1.5 text-[11px] font-bold text-amber-700">
            <span class="h-2 w-2 rounded-full bg-amber-500"></span>
            Kuota terbatas
        </span>

        <span class="inline-flex items-center gap-2 rounded-full bg-red-50 px-3 py-1.5 text-[11px] font-bold text-red-700">
            <span class="h-2 w-2 rounded-full bg-red-500"></span>
            Kuota penuh / tidak menerima
        </span>

        <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-[11px] font-bold text-slate-600">
    ...
    @if (! $guestbookProof)
        ...
        <div class="mt-5 rounded-2xl bg-amber-50 p-4 text-sm font-semibold text-amber-800">
            Upload bukti Buku Tamu untuk membuka informasi lengkap lokasi.
        </div>
        ...
        <div class="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            ...
            @foreach ($locations as $location)
                ...
                @php
                    ...
                    $quotaClass = match($location->quota_status) {
                    ...
                    <div class="flex gap-3">
                        ...
                        <div>
                            ...
                            <div class="mt-3 flex flex-wrap items-center gap-2">
                                ...
                                <span class="inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold {{ $quotaClass }}">
                                    {{ $location->quotaLabel() }}
                                </span>

                                @if($location->quota_available !== null)

                                    <span class="inline-flex items-center gap-1 rounded-full bg-ocean/10 px-2.5 py-1 text-[10px] font-bold text-ocean">

                                        <i data-lucide="users" class="h-3 w-3"></i>

                                        {{ $location->quota_available }} kuota
                            ...
                            @if($location->quota_updated_at)

                                <p class="mt-2 text-[10px] text-muted-foreground">
                                    Diperbarui
                                    {{ $location->quota_updated_at->diffForHumans() }}
                                </p>
...
{{-- =========================================================
     TAHAP 3
     SURAT PERMOHONAN + PILIHAN SERTIFIKAT
========================================================= --}}

<section id="surat-permohonan">

    <article
        class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $guestbookProof ? '' : 'pointer-events-none opacity-60' }}"
    >
        ...
        <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
            ...
            <div class="max-w-2xl">
                ...
                <p class="mt-2 text-sm leading-relaxed text-muted-foreground">
                    Pastikan surat ditujukan kepada Kepala Dinas serta mencantumkan
                    lokasi, tanggal mulai dan selesai, identitas peserta,
                    serta pernyataan permohonan penerbitan sertifikat apabila diperlukan.
                    Upload dalam format PDF maksimal 10 MB.
                </p>
            ...
            @if($requestLetter)
                ...
                <span
                    ...
                    {{
                        ($letterNeedsRevision || $automatedNeedsRevision)
                            ? 'bg-red-50 text-red-700'
                            : (
                                $letterApproved
        ...
        @if ($certificateFollowUpRequired)
            ...
            <div class="mt-6 rounded-2xl border-2 border-amber-300 bg-amber-50 p-5">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm leading-relaxed text-amber-800">
                            Surat permohonan Anda telah disetujui oleh admin,
                            tetapi surat tersebut tidak mencantumkan
                ...
                <form
                    method="POST"
                    action="{{ route('peserta.request-letter.certificate-follow-up') }}"
                    class="mt-5"
                >
                    ...
                    <fieldset>
                        ...
                        <div class="mt-3 space-y-3">
                            ...
                            <label
                                class="flex cursor-pointer items-start gap-3 rounded-2xl border-2 border-amber-200 bg-white p-4 transition hover:border-ocean hover:bg-ocean/[0.03]"
                            >

                                <input
                                    type="radio"
                                    name="certificate_follow_up_choice"
                                    value="upload_again"
                                    required
                                ...
                                <span>
                                    ...
                                    <span class="mt-1 block text-xs leading-relaxed text-muted-foreground">
                                        Saya ingin memperbaiki surat dan mengunggah
                                        surat baru yang mencantumkan pernyataan
                            ...
                            >
                                ...
                                <span>
                                    ...
                                    <span class="block text-sm font-bold text-navy">
                                        Lanjut tanpa upload ulang
                                    </span>

                                    <span class="mt-1 block text-xs leading-relaxed text-muted-foreground">
                                        Saya memahami bahwa surat ini tetap digunakan
                                        dan sertifikat mungkin tidak dapat diterbitkan
                    ...
                    @error('certificate_follow_up_choice')

                        <p class="mt-3 text-xs font-semibold text-destructive">
                            {{ $message }}
                        </p>
                    ...
                    <button
                        type="submit"
                        class="mt-4 inline-flex items-center gap-2 rounded-xl bg-ocean px-5 py-2.5 text-sm font-bold text-white transition hover:bg-navy"
                    >
        ...
        @elseif (
            ...
            {{-- STATUS SETELAH PESERTA MEMILIH UPLOAD ULANG --}}
            ...
            <div class="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs leading-relaxed text-muted-foreground">
                            Anda memilih untuk memperbaiki surat.
                            Silakan unggah surat baru yang mencantumkan
                            pernyataan permohonan penerbitan sertifikat.
                        </p>
        ...
        @elseif (
            $requestLetter?->review_status ===
                ParticipantApplicationDocument::REVIEW_APPROVED
            && $requestLetter?->certificate_eligible === false
            && $certificateFollowUpChoice === 'continue_without_upload'
        )

            {{-- STATUS SETELAH PESERTA MEMILIH LANJUT --}}

            <div class="mt-5 rounded-2xl border border-teal/25 bg-teal/[0.06] p-4">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs leading-relaxed text-muted-foreground">
                            Anda memilih untuk melanjutkan tanpa upload ulang.
                            Tahap berikutnya sekarang dapat diakses.
                        </p>
        ...
        @if ($canUploadLetter)
            ...
            <form
                data-request-letter-form
                method="POST"
                action="{{ route('peserta.request-letter.store') }}"
                enctype="multipart/form-data"
                class="mt-5 rounded-2xl border border-dashed border-ocean/30 bg-background p-5"
            >
                ...
                <input
                    ...
                    accept=".pdf"
                    required
                    class="mt-3 block w-full rounded-xl border border-border bg-background p-3 text-sm"
                >
                ...
                <label class="mt-4 flex items-start gap-3 text-xs leading-relaxed text-muted-foreground">
                    ...
                    Saya memastikan lokasi tujuan dan rentang tanggal
                ...
                <button
                    ...
                    type="submit"
                    class="mt-4 rounded-xl bg-ocean px-5 py-2.5 text-sm font-bold text-white transition disabled:pointer-events-none"
                >

                    {{
                        $letterNeedsRevision || $automatedNeedsRevision
                            ? 'Unggah & Periksa Ulang'
                            : 'Unggah & Periksa Surat'
                    }}
...
>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        @elseif(!$formStageUnlocked)
            ...
            <span class="rounded-full bg-slate-100 px-4 py-2 text-xs font-bold text-slate-500">
    ...
    @if ($formStageUnlocked)
        ...
        <div class="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            @else
                ...
                <form
                    method="POST"
                    action="{{ route('peserta.internship-form-proof.store') }}"
                    enctype="multipart/form-data"
                    class="rounded-[1.75rem] border border-dashed border-ocean/30 bg-background p-6"
                >
                    ...
                    <label class="mt-4 flex items-start gap-3 text-xs leading-relaxed text-muted-foreground">
                        ...
                        <input
                            type="checkbox"
                            name="internship_form_declaration"
                            value="1"
                            required
                            class="mt-0.5"
                        >

                        Saya menyatakan telah mengisi Google Form
                        sesuai jenjang pendidikan dengan data yang benar.
...
<section id="keputusan" class="space-y-6">
    ...
    <article
        class="rounded-[2rem] border border-border bg-white p-6 shadow-sm transition sm:p-8 {{
            $stageFiveUnlocked
                ? ''
                : 'pointer-events-none opacity-60'
        }}"
    >
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">
                        Pantau hasil pengajuan Anda di bagian ini.
                        Pemberitahuan juga akan muncul pada ikon lonceng
                        ketika Dinas memperbarui keputusan.
                    </p>
        ...
        @if (! $stageFiveUnlocked)
            ...
            <div class="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <p class="mt-1 text-sm leading-relaxed text-muted-foreground">
                    Unggah bukti pengisian Google Form resmi
                    pada Tahap 4 untuk membuka pemantauan keputusan.
                </p>
        ...
        @else
            ...
            <div class="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                ...
                @if($applicationAccepted)
                    ...
                    <div class="rounded-[1.75rem] border border-teal/25 bg-teal/[0.06] p-6">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs font-bold uppercase tracking-[0.16em] text-teal">
                                    Pengajuan diterima
                                </p>
                                ...
                                @if($officialStarted && $officialEnded)
                                    ...
                                    <p class="mt-2 text-sm font-bold text-teal">
                                        Periode
                                        {{ $officialStarted->translatedFormat('d F Y') }}
                                        sampai
                                        {{ $officialEnded->translatedFormat('d F Y') }}
                                    </p>

                                @endif

                                <p class="mt-2 text-sm leading-relaxed text-muted-foreground">
                                    Periksa surat balasan resmi dan kalender pelaksanaan Anda.
                                </p>
                ...
                @elseif($applicationRejected)
                    ...
                    <div class="rounded-[1.75rem] border border-red-200 bg-red-50 p-6">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="mt-2 text-sm leading-relaxed text-red-800">
                                    Baca surat balasan untuk mengetahui informasi lebih lanjut
                                    atau arahan yang perlu dilakukan.
                                </p>
                ...
                >
                    ...
                    @if($replyLetter)
                        ...
                        <p class="mt-2 text-xs leading-relaxed text-muted-foreground">
                            Dokumen balasan dari Dinas sudah tersedia.
                        </p>
                    ...
                    @else
                        ...
                        <p class="mt-2 text-sm leading-relaxed text-muted-foreground">
                            Surat balasan masih disiapkan oleh Dinas.
                            Dokumen akan tersedia untuk diunduh di sini
                            begitu admin mengunggahnya.
                        </p>
    ...
    >
        ...
        @if (! $stageSixUnlocked)
            ...
            <p class="mt-3 text-sm leading-relaxed text-muted-foreground">
                Tahap ini terbuka setelah peserta dinyatakan diterima pada Tahap 5.
            </p>
        ...
        @elseif($officialStarted && $officialEnded)
            ...
            <div class="mt-5 overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-navy via-[#123d72] to-ocean text-white">
                ...
                <div class="flex flex-col gap-5 p-5 sm:flex-row sm:items-end sm:justify-between sm:p-6">
                    ...
                    <div>
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs font-semibold text-blue-100">
                            ...
                            <span>
                                {{ $officialEnded->translatedFormat('d M Y') }}
                            </span>
                ...
                <div
                    id="internship-calendar-detail"
                    data-internship-calendar-detail
                    hidden
                    class="border-t border-white/10 bg-white p-5 text-navy sm:p-6"
                >
                    ...
                    <div class="grid gap-6 xl:grid-cols-[0.38fr_0.62fr]">
                        ...
                        <div class="rounded-2xl bg-light p-5">
                            ...
                            <p class="mt-4 text-xs leading-relaxed text-muted-foreground">
                                Tanggal pelaksanaan ditetapkan oleh admin.
                                Hubungi petugas apabila terdapat perubahan jadwal resmi.
                            </p>
                            ...
                            <div class="mt-5 grid grid-cols-7 gap-1 text-center">
                                ...
                                @foreach ($calendarDays as $calendarDay)
                                    ...
                                    @php
                                        ...
                                        $isStart =
                                            $day->isSameDay($officialStarted);

                                        $isEnd =
                                            $day->isSameDay($officialEnded);

                                    @endphp


                                    <div
                                        class="relative flex aspect-square min-h-9 items-center justify-center rounded-xl text-xs font-bold transition {{
                                            ! $inCurrentMonth
                                                ? 'text-slate-300'
                                                : (
                                                    $inInternship
                                        ...
                                        title="{{ $day->translatedFormat('l, d F Y') }}{{ $isStart ? ' — Hari pertama' : ($isEnd ? ' — Hari terakhir' : '') }}"
                                    >
                                        ...
                                        @if ($isToday)
                                            ...
                                            <span class="absolute bottom-1 h-1 w-1 rounded-full bg-teal"></span>
        ...
        @else
            ...
            <p class="mt-3 text-sm leading-relaxed text-muted-foreground">
                Menunggu admin menetapkan tanggal mulai dan selesai resmi.
            </p>

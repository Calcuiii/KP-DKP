        ...
        @else
            <div class="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                @if($applicationAccepted)
                    <div class="rounded-[1.75rem] border border-teal/25 bg-teal/[0.06] p-6"><div class="flex items-start gap-4"><span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-teal text-white"><i data-lucide="check" class="h-5 w-5"></i></span><div><p class="text-xs font-bold uppercase tracking-[0.16em] text-teal">Pengajuan diterima</p><h3 class="mt-2 text-xl font-extrabold text-navy">Selamat, Anda dapat melanjutkan ke tahap pelaksanaan.</h3>@if($officialStarted && $officialEnded)<p class="mt-2 text-sm font-bold text-teal">Periode {{ $officialStarted->translatedFormat('d F Y') }} sampai {{ $officialEnded->translatedFormat('d F Y') }}</p>@endif<p class="mt-2 text-sm leading-relaxed text-muted-foreground">Periksa surat balasan resmi dan kalender pelaksanaan Anda.</p></div></div></div>

...
<section id="persiapan">
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8">
        <div class="flex items-start gap-4">
            ...
            <div><p class="text-xs font-bold uppercase tracking-[0.18em] text-ocean">Tahap 1</p><h2 class="mt-1 text-xl font-bold">Isi dan buktikan Buku Tamu</h2><p class="mt-2 text-sm leading-relaxed text-muted-foreground">Pengisian dilakukan secara individu. Setelah selesai, unggah screenshot atau PDF sebagai bukti.</p></div>
        </div>
...
<section id="surat-permohonan">
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $guestbookProof ? '' : 'pointer-events-none opacity-60' }}">
        <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
            <div class="max-w-2xl"><p class="text-xs font-bold uppercase tracking-[0.18em] text-ocean">Tahap 3</p><h2 class="mt-2 text-2xl font-bold">Siapkan dan upload surat permohonan</h2><p class="mt-2 text-sm leading-relaxed text-muted-foreground">Pastikan surat ditujukan kepada Kepala Dinas serta mencantumkan lokasi, tanggal mulai dan selesai, dan identitas peserta. Upload dalam format PDF maksimal 10 MB.</p></div>
...
<section id="google-form" class="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8 {{ $letterApproved ? '' : 'pointer-events-none opacity-60' }}">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex items-start gap-4"><span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl {{ $internshipFormCompleted ? 'bg-teal text-white' : 'bg-ocean text-white' }}"><i data-lucide="clipboard-check" class="h-5 w-5"></i></span><div><p class="text-xs font-bold uppercase tracking-[0.18em] text-teal">Magang / PKL / KP · Tahap 4</p><h2 class="mt-2 text-2xl font-bold">Isi dan buktikan Google Form resmi</h2></div></div>
    ...
    @if($letterApproved)
        <div class="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            <div class="rounded-[1.75rem] bg-gradient-to-br from-navy to-ocean p-6 text-white">
                ...
                <h3 class="mt-5 text-lg font-bold">Google Form pendaftaran resmi</h3>
            ...
            @if($internshipFormProof)
                <div class="rounded-[1.75rem] border border-teal/25 bg-teal/[0.05] p-6">
                    <div class="flex items-start gap-4"><span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-teal/10 text-teal"><i data-lucide="file-check" class="h-5 w-5"></i></span><div><p class="text-sm font-bold">Bukti pengisian sudah dikirim</p><p class="mt-1 text-xs text-muted-foreground">{{ $internshipFormProof->original_name }} · {{ $internshipFormProof->created_at->format('d M Y, H:i') }}</p><a href="{{ route('peserta.document.download', $internshipFormProof) }}" class="mt-4 inline-flex items-center gap-2 text-xs font-bold text-ocean"><i data-lucide="download" class="h-4 w-4"></i>Lihat bukti</a></div></div>
            ...
            @else
                <form method="POST" action="{{ route('peserta.internship-form-proof.store') }}" enctype="multipart/form-data" class="rounded-[1.75rem] border border-dashed border-ocean/30 bg-background p-6">
                    @csrf
                    <label class="text-sm font-bold">Upload screenshot bukti pengisian</label><p class="mt-1 text-xs leading-relaxed text-muted-foreground">Gunakan screenshot halaman yang menyatakan respons berhasil dikirim. Format JPG, PNG, atau PDF, maksimal 5 MB.</p>
...
<section id="keputusan" class="space-y-6">
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm transition sm:p-8 {{ $stageFiveUnlocked ? '' : 'pointer-events-none opacity-60' }}">
        ...
        @if (! $stageFiveUnlocked)
            <div class="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5"><p class="text-sm font-bold text-navy">Tahap belum tersedia</p><p class="mt-1 text-sm leading-relaxed text-muted-foreground">Unggah bukti pengisian Google Form resmi pada Tahap 4 untuk membuka pemantauan keputusan.</p></div>
        @else
            <div class="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                @if($applicationAccepted)
                    <div class="rounded-[1.75rem] border border-teal/25 bg-teal/[0.06] p-6"><div class="flex items-start gap-4"><span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-teal text-white"><i data-lucide="check" class="h-5 w-5"></i></span><div><p class="text-xs font-bold uppercase tracking-[0.16em] text-teal">Pengajuan diterima</p><h3 class="mt-2 text-xl font-bold text-navy">Selamat, Anda dapat melanjutkan ke tahap pelaksanaan.</h3>@if($officialStarted && $officialEnded)<p class="mt-2 text-sm font-bold text-teal">Periode {{ $officialStarted->translatedFormat('d F Y') }} sampai {{ $officialEnded->translatedFormat('d F Y') }}</p>@endif<p class="mt-2 text-sm leading-relaxed text-muted-foreground">Periksa surat balasan resmi dan kalender pelaksanaan Anda.</p></div></div></div>
                @elseif($applicationRejected)
                    <div class="rounded-[1.75rem] border border-red-200 bg-red-50 p-6"><div class="flex items-start gap-4"><span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-red-600 text-white"><i data-lucide="x" class="h-5 w-5"></i></span><div><p class="text-xs font-bold uppercase tracking-[0.16em] text-red-700">Pengajuan belum dapat diterima</p><h3 class="mt-2 text-xl font-bold text-navy">Silakan periksa keputusan resmi dari Dinas.</h3><p class="mt-2 text-sm leading-relaxed text-red-800">Baca surat balasan untuk mengetahui informasi lebih lanjut atau arahan yang perlu dilakukan.</p></div></div></div>
                @else
                    <div class="rounded-[1.75rem] bg-gradient-to-br from-navy to-ocean p-6 text-white"><span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/15"><i data-lucide="clock" class="h-5 w-5"></i></span><p class="mt-5 text-xs font-bold uppercase tracking-[0.16em] text-blue-200">Status saat ini</p><h3 class="mt-2 text-xl font-bold">{{ $replyLetter ? 'Surat balasan telah dikirim' : 'Menunggu keputusan Dinas' }}</h3><p class="mt-2 text-sm leading-relaxed text-blue-100">{{ $replyLetter ? 'Silakan periksa dan unduh surat balasan resmi dari Dinas di samping.' : 'Pantau notifikasi portal untuk keputusan atau surat balasan dari Dinas.' }}</p></div>
                @endif

                <div id="surat-balasan" class="rounded-[1.75rem] border border-border bg-background p-6">
                    <p class="text-sm font-bold text-navy">Surat balasan resmi</p>
                    @if($replyLetter)
    ...
    <article class="rounded-[2rem] border border-border bg-white p-6 shadow-sm transition sm:p-8 {{ $stageSixUnlocked ? '' : 'pointer-events-none opacity-60' }}">
        <div class="flex items-start justify-between gap-3"><div><p class="text-xs font-bold uppercase tracking-[0.18em] text-ocean">Tahap 6</p><h2 class="mt-2 text-xl font-bold">Kalender pelaksanaan</h2></div>@unless($stageSixUnlocked)<span class="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-slate-500"><i data-lucide="lock" class="h-4 w-4"></i></span>@endunless</div>
        @if (! $stageSixUnlocked)
        ...
        @elseif($officialStarted && $officialEnded)
            <div class="mt-5 overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-navy via-[#123d72] to-ocean text-white">
                <div class="flex flex-col gap-5 p-5 sm:flex-row sm:items-end sm:justify-between sm:p-6">
                    <div>
                        <p class="text-3xl font-bold">{{ $daysRemaining }} hari</p>
                    ...
                    <button type="button" data-internship-calendar-toggle aria-expanded="false" aria-controls="internship-calendar-detail" class="inline-flex w-fit items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-ocean shadow-lg shadow-navy/20 transition hover:-translate-y-0.5 hover:bg-blue-50">
                    ...
                    <div class="grid gap-6 xl:grid-cols-[0.38fr_0.62fr]">
                        ...
                        <div class="rounded-2xl border border-border bg-white p-4 shadow-sm sm:p-5">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div><p class="text-xs font-semibold text-muted-foreground">Kalender pelaksanaan</p><h3 class="mt-1 text-lg font-bold">{{ $calendarMonth->translatedFormat('F Y') }}</h3></div>

...
@section('content')
    @php
        ...
        $applicationCatalog = $applications->map(function ($item) use ($application) {
            return [
                'id' => $item->id,
                'service_type' => $item->service_type,
                'label' => $item->serviceLabel(),
                'status' => $item->status,
                'active' => $application && $application->id === $item->id,
            ];
        })->values();
        ...
        <div class="mx-auto flex max-w-[90rem] gap-6 px-5 py-7 sm:px-8 lg:px-10">
            ...
            <div class="participant-dashboard-content min-w-0 flex-1 space-y-6">
                ...
                @if ($applications->isNotEmpty())
                    <section class="rounded-[2rem] border border-border bg-white p-5 shadow-sm sm:p-6">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-[0.18em] text-ocean">Layanan aktif</p>
                                <h2 class="mt-2 text-xl font-extrabold text-navy">Pilih aplikasi yang ingin Anda kelola</h2>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                @foreach ($applications as $serviceApplication)
                                    <a href="{{ route('peserta.application.select', $serviceApplication) }}" class="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold transition {{ $application && $application->id === $serviceApplication->id ? 'border-ocean bg-ocean text-white shadow-sm' : 'border-border bg-light text-muted-foreground hover:border-ocean/40 hover:text-ocean' }}">
                                        <span class="inline-flex h-6 w-6 items-center justify-center rounded-full {{ $application && $application->id === $serviceApplication->id ? 'bg-white/20 text-white' : 'bg-ocean/10 text-ocean' }}">
                                            <i data-lucide="{{ $serviceApplication->service_type === \App\Models\ParticipantApplication::SERVICE_WOPPS ? 'microscope' : 'graduation-cap' }}" class="h-3.5 w-3.5"></i>
                                        </span>
                                        {{ $serviceApplication->serviceLabel() }}
                                    </a>
                                @endforeach
                            </div>
                        </div>
                ...
                @unless (($isInternshipExecution ?? false))
                ...
                @if (($isInternshipExecution ?? false))
                ...
                @else
                    ...
                    @if (($application ?? null) && $application->service_type === \App\Models\ParticipantApplication::SERVICE_MAGANG_PKL)

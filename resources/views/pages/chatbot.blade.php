...
@section('content')
    <div
        class="flex min-h-screen bg-light font-sans text-navy"
        data-chatbot-app
        data-history-url="{{ route('chatbot.api.history') }}"
        data-send-url="{{ route('chatbot.api.messages.send') }}"
        data-conversation-url-template="{{ url('/api/chatbot/conversations/__CONVERSATION__') }}"
        data-feedback-url-template="{{ url('/api/chatbot/messages/__MESSAGE__/feedback') }}"
    >
        ...
        <main class="flex min-h-screen min-w-0 flex-1 flex-col">
            ...
            <header class="flex h-16 items-center border-b border-border bg-white px-4 sm:px-6">
                <button
                    ...
                    class="rounded-lg p-2 text-navy transition hover:bg-secondary lg:hidden"
            ...
            <section
                data-chat-empty
                class="flex flex-1 flex-col items-center justify-center overflow-y-auto px-4 py-10 text-center"
            >
                <div class="w-full max-w-xl">
                    ...
                    <div class="mt-6 grid grid-cols-1 gap-2 text-left sm:grid-cols-2">
                        <button
                            type="button"
                            data-chat-suggested="Apa saja persyaratan pengajuan magang?"
                        ...
                        <button
                            type="button"
                            data-chat-suggested="Bagaimana alur pengajuan KP?"
                            class="rounded-xl border border-border bg-white px-4 py-3 text-sm text-navy shadow-sm transition hover:border-ocean hover:shadow-md"
                        >
                        ...
                        <button
                            type="button"
                            data-chat-suggested="Berapa lama proses pengajuan?"
                            class="rounded-xl border border-border bg-white px-4 py-3 text-sm text-navy shadow-sm transition hover:border-ocean hover:shadow-md"
                        >
                        ...
                        <button
                            type="button"
                            data-chat-suggested="Dokumen apa saja yang harus disiapkan?"
                            class="rounded-xl border border-border bg-white px-4 py-3 text-sm text-navy shadow-sm transition hover:border-ocean hover:shadow-md"
                        >
            ...
            <section
                data-chat-messages
                class="hidden flex-1 overflow-y-auto py-4"
            >
                <div
                    data-chat-message-list
                    class="mx-auto flex max-w-4xl flex-col gap-6 px-4"
                ></div>
            </section>
 
            <div
                data-chat-error
                class="hidden border-t border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600"
            ></div>
